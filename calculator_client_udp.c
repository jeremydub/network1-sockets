...
int main() {
    ...
    int sock = create_client_local_udp_socket(7777, &server);
	...
	int recv_n;
    // Contiendra la taille de la structure contenant l'adresse du client
    int slen;
	...
	while(1)
    {
        ...
        // pointeur 'buffer'
        if( sendto(sock, buffer, 9, 0,(struct sockaddr *) &server, sizeof(server)) < 0)
        {
        ...
        // bits, i.e. 4 octets
        if( (recv_n = recvfrom(sock, &resultat , 4, 0,(struct sockaddr *) &server, &slen)) < 0)
        {
